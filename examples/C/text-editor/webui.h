...
WEBUI_EXPORT void webui_set_file_handler(
    size_t window, const void *(*handler)(const char *filename, int *length));
...
WEBUI_EXPORT void webui_set_icon(size_t window, const char *icon,
...
// Run a JavaScript, and get the response back (Make sure your local buffer can
// hold the response).
WEBUI_EXPORT bool webui_script(size_t window, const char *script,
                               size_t timeout, char *buffer,
...
// Base64 encoding. Use this to safely send text based data to the UI. If it
...
// Safely allocate memory using the WebUI memory management system. It can be
// safely free using webui_free().
WEBUI_EXPORT void *webui_malloc(size_t size);

// -- Interface -----------------------
// Bind a specific html element click event with a function. Empty element means
// all events. This replace webui_bind(). The func is (Window, EventType,
// Element, Data, EventNumber)
WEBUI_EXPORT size_t webui_interface_bind(size_t window, const char *element,
                                         void (*func)(size_t, size_t, char *,
                                                      char *, size_t));
// When using `webui_interface_bind()` you may need this function to easily set
// your callback response.
WEBUI_EXPORT void webui_interface_set_response(size_t window,
                                               size_t event_number,
                                               const char *response);
...
WEBUI_EXPORT size_t webui_interface_get_bind_id(size_t window,
                                                const char *element);
